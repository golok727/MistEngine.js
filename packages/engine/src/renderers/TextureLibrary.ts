import {loadImageAsync} from '@mist/utils'
import {MistTexture, TextureFactory} from './Texture'
...
export default class MistTextureLibrary {
  ...
  public static async Create(app: MistApp, tImageUrl: string) {
    ...
    if (!this.preloadedImages.get(tImageUrl)) await this.PreloadImage(tImageUrl)

    if (exists) return exists

    const texture = TextureFactory.Create(tImageUrl, app.getRenderer())
